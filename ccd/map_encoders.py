...
class BasicMapEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.layer1 = nn.Conv2d(
            in_channels=n_semantic_classes ,  # one extra for ignore index
        ...
        self.depthwise_1 = nn.Conv2d(in_channels=64,
                                    out_channels=64,
                                   kernel_size=3, padding=1,
                                   stride=1, groups=64)
        # self.depthwise_2 = nn.Conv2d(in_channels=64,
        #                              out_channels=64,
        #                              kernel_size=3, padding=1,
        #                              stride=1, groups=in_channels),
        self.pointwise = nn.Conv2d(in_channels=64,
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Conv2d(self.out_channels[0], self.out_channels[0] // 16, 1, bias=False),
            # inplace=True直接替换，节省内存
            nn.ReLU(inplace=True),
            # nn.Linear(channel // reduction, channel,bias=False)
            nn.Conv2d(self.out_channels[0] // 16, self.out_channels[0], 1, bias=False)
        )
    ...
    def forward(self, gt_semantic_seg_pre):
        #通道数量
        if gt_semantic_seg_pre.ndim == 5:# 原来时4
            # print(gt_semantic_seg_pre.size())
            gt_semantic_seg_pre = gt_semantic_seg_pre.squeeze(1)\
                # .view(20,512,512)

            # print(gt_semantic_seg_pre.size())
        B, H, W,_ = gt_semantic_seg_pre.shape
        ...
        with torch.no_grad():
            ...
            b,c,h,w = _gt_semantic_seg_pre.shape
            if(c!=5):
                one_hot = _gt_semantic_seg_pre.permute(0,2,1,3).reshape(
                    B,5, H, W).float()
            else:
                one_hot = _gt_semantic_seg_pre.float()
...
class HighLevelMapEncoder(BasicMapEncoder):
    '''
    Map encoder with consolidated classes.合并类别的地图编码器
    '''
    def __init__(
        self,  
        ignore_index, 
...
class LowResMapEncoder(BasicMapEncoder):
    '''
    Map encoder with downsampling to mimic low res map info.地图编码器可降低采样率，以模拟低分辨率地图信息
    '''
